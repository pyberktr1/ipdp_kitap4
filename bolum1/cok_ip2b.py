# cok ip uzerinde uygulama ornegi 2b
# for döngüleri ile çoklu ip yönetimi ve iplerde uyuma (sleep) olayı
...
def gorev(id, bas):
    ...
    for _ in range(2):
        ...
        # I/O islemleri esnasında islemcinin bosta bekledigi uzun gecikmeler olur
        # I/O islemlerine ornek: web sorgulamaları, disk islemleri v.s.
        sleep(0.1)
        # bu bolumde sorgulamanın ne kadar surdugu hesaplanmakta
        an = perf_counter()
        print(f"gorev {id} : gecen sure : {an - bas: 2.3f} \n", end="")
    # burada 3 numaralı gorev uyutuluyor
    # uyutma islemi islemci zamanından calmaz
    # bir ipi beklemeye almak zorunlu ise sleep() metodu ile uyutma yapılmalıdır
    while (id == 3):
        sleep(0.1)
