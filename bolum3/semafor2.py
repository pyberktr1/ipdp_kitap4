...
# iki ayrı semafor kullanimi ile indirme ve açık dosya sayisi 
# ayrı ayrı sinirlandiriliyor
...
# indirme limiti
max_indirme_limiti = 3
# semafor tanimlaniyor
semafor_web = threading.Semaphore(max_indirme_limiti)

# dosya limiti
max_dosya_limiti = 2
# semafor tanimlaniyor
semafor_dosya = threading.Semaphore(max_dosya_limiti)
...
def im_indir(url):
    ...
    # semafor_web ile indirme limiti getiriliyor
    with semafor_web:
        # web kaynagi acilip indirme gerceklestiriliyor
        print(f"\033[91m{d_ad} indirme basliyor...\033[0m")
        ...
        if durum == 200: # getirme islemi basarili
            ...
            data = cevap.content
            print(f"{d_ad} indirme tamam...")
        else:
            print(f"\033[91mHata!: {url} :: kod={durum} \033[0m")
    
    # dosya islem bolumu   
    if durum == 200: # getirme basarili ise dosyaya isle
        # ikinci semafor acik dosya sayisini sinirlar
        with semafor_dosya:
            print(f"\033[92m{d_ad} aciliyor..\033[0m")
            # dosyalari biriktirmek icin kasitli gecikme veriyoruz
            time.sleep(3)
            # resim verisi diske kaydediliyor
            with open(f"./resim/{d_ad}", 'wb') as im_d:
                im_d.write(data)
                print(f'\033[95m{d_ad} kaydedildi...\033[0m')
