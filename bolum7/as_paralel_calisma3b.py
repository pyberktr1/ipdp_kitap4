# bu ornek korolarin paralel (asenkron) calismasi esnasinda
# beklenecek gorevleri gather() ile korolardan dogrudan olusturmayi gosterir
...
async def ana():
    ...
    # asyncio.gather() ile korolarin gorev beklemesini birlestiriyoruz
    # gorev olusturma koro nesnelerinden otomatik olarak ve dogrudan
    # gather() metodu tarafindan gerceklestirilir
    kare, kup = await asyncio.gather(as_kare_al(3), as_kup_al(2))
    print(f"kare: {kare}")
    print(f"kup: {kup}")
