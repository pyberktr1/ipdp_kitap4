...
def temizlik(no, isim, camasir, bulasik, cmak, bmak):
    ...
    # tum camasir ve bulasik isleri tamamlanana kadar devam et
    while (camasir[no] > 0) or (bulasik[no] > 0):
        # camasir sirasina gir
        if camasir[no]>0:# camasir yoksa bosa sira bekleme
            # bos camasir makinesi yok, bekle
            print(f"\033[91m{isim} camasir makinesi bekliyor...\033[00m")
            cevap = cmak.acquire(block=True,timeout=1)
            if not cevap:# bir zamanasimi var
                print(f"\033[94m{isim}: camasir sirasinda zamanasimi oldu.\033[00m")
            else:
                try:
                    ...
                    cmak.release()
                    print(f"{isim}::{cmak.get_value()}")
                    
        # bulasik sirasina gir
        if bulasik[no]>0:# bulasik yoksa bosa sira bekleme
            # bos bulasik makinesi yok, bekle
            print(f"\033[91m{isim} bulasik makinesi bekliyor...\033[00m")
            cevap = bmak.acquire(block=True,timeout=1)
            if not cevap:# bir zamanasimi var
                print(f"\033[94m{isim}: bulasik sirasinda zamanasimi oldu.\033[00m")
            else:
                try:
                    ...
                    bmak.release()
                    print(f"{isim}::{bmak.get_value()}")
            
    print(f"{isim} tum isleri toplam {time.perf_counter() - bas:1.3f} saniyede tamamladi.")
...
if __name__ == "__main__":
    ...
    for i in range(len(isimler)):
        ...
        bulasik[i] = 5#(random.uniform(2,5))
